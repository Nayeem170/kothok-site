...
/// Refresh via `--features screenshot` build, header centred.
...
// Pixel-proportional to the real 1072x1448 panel.
const PANEL_W = 1072;
const PANEL_H = 1448;
const SCREEN_H = 2.48;
...
/// Imperative load (no Suspense boundary in the Canvas).
    ...
    Promise.all(
      SCREEN_STATES.map((state) =>
        loader
          ...
          // best-effort: a missing capture leaves the panel paper-white
          .catch(() => null),
      ),
    ).then((entries) => {
      if (cancelled) {
...
export function Device({ theme, reducedMotion }: DeviceProps) {
  ...
  const koboTex = useMemo(
    () => makeKoboLogoTexture(KOBO_COLOR[theme], maxAniso),
    [theme, maxAniso],
  );
  ...
  useFrame((state) => {
    ...
    if (flashMat.current && !reducedMotion) {
      ...
      flashMat.current.opacity =
        since < FLASH_DURATION_SEC ? FLASH_PEAK_OPACITY * (1 - since / FLASH_DURATION_SEC) : 0;
    }
